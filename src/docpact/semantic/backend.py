...
class OpenAICompatBackend:
    ...
    def __init__(self, *, api_base: str, model: str, api_key: str, timeout: float = 90.0) -> None:
    ...
    def complete(self, system: str, user: str) -> str:
        ...
        try:
            ...
            if exc.code == 429:
                raise SemanticError(f"rate-limited (HTTP 429): {detail}") from exc
            if exc.code in (401, 403):
                raise SemanticError(
                    f"auth failed (HTTP {exc.code}); check the credential in the "
                    f"configured api_key_env has access: {detail}"
                ) from exc
            raise SemanticError(f"HTTP {exc.code}: {detail}") from exc
...
def make_backend(config: SemanticConfig) -> LLMBackend:
    ...
    if config.backend == "openai-compat":
        ...
        return OpenAICompatBackend(api_base=config.api_base, model=config.model, api_key=api_key)
    raise SemanticError(

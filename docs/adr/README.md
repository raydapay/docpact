...
| [013](ADR-013-module-level-semantic-scan.md) | Module-level semantic scan (SEM002) — model-sensitive, symbols-scoped | Accepted | Build `SEM002` (weak module docstring: scope + orientation), opt-in via `scan_modes=["module"]`, input = docstring + public symbols (no `context_files`). Spike: gpt-4o-mini 71%/43% FP → gpt-4o 0% FP with a consistency-not-completeness prompt. Model-sensitive; weak models not recommended. |
| [014](ADR-014-cli-accretion-point.md) | `cli.py` is the accretion point — defer `CheckEngine` extraction to the next REG010/crossfile change | Accepted | Don't extract speculatively; bind the refactor to the next change touching the REG010/cross-file seam (or `cli.py` > ~2000 lines). Records the watched module so a future session inherits the intent. |
| [015](ADR-015-sem-stable-intent-not-behavior.md) | SEM codes guarantee stable intent, not stable behavior | Accepted | §18.1's stable-behavior guarantee does not extend to SEM; behavior tracks (model, prompt). Can't pin model; do pin prompt; surface model id + prompt fingerprint in output. Rubric-only override is the designed (not-built) mechanism — whole-prompt override rejected as a silent footgun. |
| [016](ADR-016-sphinx-format-and-custom-sections.md) | Sphinx/reST format with docpact-owned custom sections | Accepted — deferred (unscheduled) | `format = "sphinx"` would back `:param`/`:return`/`:raises` with griffe, **plus** a docpact-owned recovery pass for Constraints/MCP/Mutates/Notes — griffe drops those under Sphinx, so Tier-3 reST is unsatisfiable without it. Designed, not built; no release scheduled. |
| [017](ADR-017-reflective-class-based-tool-detection.md) | Reflective class-based agent-tool detection via tier assignment | Accepted — deferred (unscheduled) | Detecting `class XTool(Tool): def apply(...)` is a **tier-assignment** problem, not a registry one — a syntactic (path + base/suffix + method) matcher floors the tier; existing function-level rules do the rest. No `name_from`/`description_from`, no class-docstring rule, no inheritance resolution. Designed, not built; coupled to ADR-016. |
